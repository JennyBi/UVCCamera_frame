...
int UVCPreview::setFrameCallback(JNIEnv *env, jobject frame_callback_obj, int pixel_format) {
	LOGE("setFrameCallback000000000000000");
	ENTER();
	LOGE("setFrameCallback111111111111");
	pthread_mutex_lock(&capture_mutex);
	{
		...
		LOGE("setFrameCallback22222222222222");
		if (!env->IsSameObject(mFrameCallbackObj, frame_callback_obj))	{
			iframecallback_fields.onFrame = NULL;
			LOGE("setFrameCallback3333333333333333");
			if (mFrameCallbackObj) {
				env->DeleteGlobalRef(mFrameCallbackObj);
				LOGE("setFrameCallback4444444444444");
			}
			...
			if (frame_callback_obj) {
				...
				if (LIKELY(clazz)) {
					iframecallback_fields.onFrame = env->GetMethodID(clazz,
						...
						LOGE("setFrameCallback5555555555555");
				} else {
...
int UVCPreview::prepare_preview(uvc_stream_ctrl_t *ctrl) {
 ...
 //   	result = uvc_get_stream_ctrl_format_size_fps(mDeviceHandle, ctrl,
//    		!requestMode ? UVC_FRAME_FORMAT_UYVY : UVC_FRAME_FORMAT_MJPEG,
 //   		requestWidth, requestHeight, requestMinFps, requestMaxFps
//    	);
    	LOGE("requestMode = %d",requestMode);
    	LOGE("requestWidth = %d",requestWidth);
    	LOGE("requestHeight = %d",requestHeight);
...
void UVCPreview::do_capture_callback(JNIEnv *env, uvc_frame_t *frame) {
	ENTER();
    LOGE("do_capture_callback 111111111111111");
	if (LIKELY(frame)) {
		uvc_frame_t *callback_frame = frame;
		LOGE("do_capture_callback 2222222222222");
		if (mFrameCallbackObj) {
			...
			LOGE("do_capture_callback 333333333333333");
			jobject buf = env->NewDirectByteBuffer(callback_frame->data, callbackPixelBytes);
			env->CallVoidMethod(mFrameCallbackObj, iframecallback_fields.onFrame, buf);
			env->ExceptionClear();
			env->DeleteLocalRef(buf);
			LOGE("do_capture_callback 4444444444444");
		}
 SKIP:
		{recycle_frame(callback_frame);
		LOGE("do_capture_callback 55555555555555");
		}
	}
	EXIT();
}
