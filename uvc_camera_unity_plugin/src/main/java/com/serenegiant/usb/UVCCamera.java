...
public class UVCCamera {
    ...
    public UVCCamera() {
		...
		Log.d(TAG, "UVCCamera: mNativePtr= "+mNativePtr);
    	mSupportedSize = null;
	}
    ...
    public void open(final UsbControlBlock ctrlBlock) {
		...
		int result = nativeConnect(mNativePtr,
    	...
    	if (mNativePtr != 0 && TextUtils.isEmpty(mSupportedSize)) {
			...
			Log.d(TAG, "mSupportedSize=== "+mSupportedSize);
    	}
    ...
    public void setFrameCallback(final IFrameCallback callback, final int pixelFormat) {
    	if (mNativePtr != 0) {
        	int res = nativeSetFrameCallback(mNativePtr, callback, pixelFormat);
			Log.d(TAG, "nativeSetFrameCallback: "+res);
    	}
    }
